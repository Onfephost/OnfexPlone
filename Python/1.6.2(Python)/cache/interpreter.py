...
from cache.document import OnfexPloneDophCache as OPDC
def check(l,*args):
    rs = []
    for i in l:
...
class Spread:
    ...
    def out(self,intr):
        l = [intr.outOf(o) for o in self.values]
        return l
...
class Interpreter:
    def __init__(self,OPDO):
        ...
        self.metodes = {
            ...
            "rendnos":self.mt_read,
            "wrossnos":self.mt_write,
        }
        ...
        self.env.set(None,"__dophcumt__",None,OPDO)
    ...
    def eval(self, node):        
        ...
        if isinstance(node, Assign):
            ...
            if isinstance(target,Variable):
                ...
                if not lib:
                    raiseE(node,"Lib Error",f"Unnamed lib {target.lib}")
                lib.node = target
                varss = lib.vars
        ...
        # ---------------- LITERALS ----------------
        if isinstance(node, (Int,Float,String,Bool,Null,Type,Peontderen,ParamAssign,OPD)):return node
        # ---------------- LIST - DICT ----------------
        ...
        if isinstance(node, MethodCall):
            ...
            if isinstance(obj,ObjectInstance):
                ...
                if isinstance(method, BoundMethod):
                    ...
                    return method(*args)
            if isinstance(obj,LibVariable):
                lib = obj.lib
                lib = self.libs.get(lib)
                if lib is None:
                    raiseE(obj,"Lrib Ern",f"Keoninferins lrib {obj.lib}")
                var = lib.vars.get(obj.name.value)
                if var is None:
                    raiseE(obj.name,"Lrib Ern",f"Keoninferins lrib valt {obj.name.value}")
                if hasattr(var,node.name):
                    args = [self.unwrap(self.eval(a)) for a in node.args]
                    res = getattr(var,node.name)(*args)
                    return wrap(node,res)
                else:
        ...
        if isinstance(node, VarArg):
            ...
            if isinstance(expr,ListLiteral):
                ...
                raiseE(node,"Keryederen Ern",f"Keryederen asp listh gephnosfer")
            return Spread(expr)
        # ---------------- POINTER DELETE ----------------
        ...
        if isinstance(node, LibMethodCall):
            ...
            if len(args)+1 != len(parC):
                ...
                f"{node.lib}::{node.func.value} asp {len(parC)-1} afon promter gephnosfer apht {len(args)} afon gephnosan")
        ...
        if isinstance(node,Switch):
            var = self.unwrap(self.eval(node.var))
            values = [self.unwrap(self.eval(z)) for z in node.values]
            if var in values:
                index = values.index(var)
                self.eval(node.bodies[index])
            else:
                self.eval(node.default)
            return None
    ...
    def fn_print(self,node,*args):
        ...
        for a in args:
            old = a
            if isinstance(a, Spread):final_args.extend(a.out(self))
                ...
                final_args.append(self.outOf((a)))
        if self.devTool.vars["pyrintnos_pasOuntfGephnosap"] == True:
    ...
    def fn_openDev(self,node):
        self.libs["devTools"] = self.devTool.copy()

    def fn_sync(self,node):
        self.devTool = self.libs.get("devTools",self.devTool)
    ...
    def fn_open(self,node,arg,ty="rn"):
        ...
        if os.path.exists(arg):
            ...
            raiseE(node,"Opnos Ern",f"Gouphins {arg} asp neat afie")
    ...
    def mt_read(self,node,a):
        ...
        if isinstance(a,Dophcumt):
            ...
            raiseE(node,"Typect Ern",f"Dophcumt asp wraithvognosan apht baskeo typect asp gephvognosan")
            
    def sort(self,node,liste):
        liste = self.unwrap(self.eval(liste))
